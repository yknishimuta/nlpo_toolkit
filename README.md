...
Use `nlpo concordance` to build KWIC/concordance output from a token artifact
or a legacy diagnostic trace TSV generated by `nlpo count-vocabula`.

With one work item, `trace.path: output/trace.tsv` writes `output/trace.tsv`.
With multiple groups or per-file work items, each trace is written separately
using the work item label, for example `output/trace_full.tsv` and
`output/trace_cena.tsv`. Pass the trace file for the group you want to inspect.
...
The token artifact records all NLP tokens with schema metadata in
`output/tokens.meta.json`. Legacy diagnostic traces can be partial when
`trace.only_keys` or `trace.max_rows` is used, so use token artifacts for
complete downstream reuse. If the input contains file, group, or sentence
metadata, those columns are included in the concordance output.
...
legacy diagnostic trace TSV, or text files listed in `config/groups.config.yml`.

Token artifact input is the recommended reusable workflow because it can use
either the `token` or `lemma` column and is complete. Legacy trace input remains
supported for compatibility:

For multi-group count runs, point `--trace` at the specific generated trace file,
such as `output/trace_full.tsv` or `output/trace_cena.tsv`.
...
are counted. Token artifact and legacy trace input are different: they read the
existing TSV directly and do not re-run cleaner or text normalization. Config
input supports `--field token`; use token artifact or trace input for lemma
n-grams.
...
`trace` is a diagnostic output and may be filtered or truncated. `artifacts.tokens`
is the formal complete token artifact; if both are enabled, use different paths.
