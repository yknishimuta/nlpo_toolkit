...
def test_config_reference_consumers_do_not_resolve_config_strings() -> None:
    forbidden = {
        ...
        Path("nlpo_toolkit/corpus_analysis/postprocessing/service.py"): "plan.config.dictcheck.lemma_normalize",
...
def test_run_reporting_does_not_rebuild_config_inventory() -> None:
    source = Path("nlpo_toolkit/corpus_analysis/reporting/metadata.py").read_text(
        encoding="utf-8"
    )
