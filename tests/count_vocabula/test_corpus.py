...
from nlpo_toolkit.count_vocabula.config import ensure_app_config
from nlpo_toolkit.count_vocabula.corpus import (
...
def test_prepare_corpus_text_uses_supplied_patterns_without_reload(tmp_path: Path, monkeypatch) -> None:
    ...
    patterns = tuple(__import__("nlpo_toolkit.count_vocabula.ref_tags", fromlist=["load_ref_tag_patterns"]).load_ref_tag_patterns(ref_path))

    import nlpo_toolkit.count_vocabula.corpus as corpus_mod
...
def test_count_features_and_ngram_config_receive_same_prepared_text(tmp_path: Path, monkeypatch) -> None:
    import nlpo_toolkit.count_vocabula.features as features_mod
    import nlpo_toolkit.count_vocabula.ngram as ngram_mod
