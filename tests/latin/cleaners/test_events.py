...
from nlpo_toolkit.latin.cleaners.events import write_ref_events
from nlpo_toolkit.latin.cleaners.models import RefEvent, RuleReference


def test_events_writer_preserves_schema_and_replaces_previous_run(tmp_path: Path) -> None:
    path = tmp_path / "nested/events.tsv"
    event = RefEvent("d", "corpus_corporum", "r", "substitute", 2, 3, RuleReference("k", "a", "w", "l"), "text")
    write_ref_events(path, (event, event))
    write_ref_events(path, (event,))
